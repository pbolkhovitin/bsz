...
| MikroTik CRS328 | ? | ? | ? | ? | ? | ? | ? |
| D-Link (bsz-sw-XX) | telnet | web 80 | — | — | — | — | BSZ-m0n1t0r ✅ |
| MikroTik CRS328 (bsz-sw-01) | — | web | — | — | — | — | BSZ-m0n1t0r ✅ |
...
## Методика безопасного telnet-доступа

Описан в разделе «Методика безопасного доступа по Telnet (с использованием vault-bsz)» ниже:
смена дефолтных паролей (admin/admin, admin/пусто) на уникальные, хранение в Vault
(`bsz/switches/<name>`), ограничение доступа (MikroTik firewall: только 172.17.101.0/24),
порядок внедрения, аудит. Команды: D-Link — `config account admin password <new>` + `save`.
